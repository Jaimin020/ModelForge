...
export const ImageInputConfig = ({ onSaveReady, selectedNode }) => {
  const [inputParams, setInputParams] = useState({
    Folder: selectedNode?.parameters?.['Folder'] || '',
    'Number of Classes': selectedNode?.parameters?.['Number of Classes'] || '',
    'Total Images': selectedNode?.parameters?.['Total Images'] || '',
  });
  const [detectedClasses, setDetectedClasses] = useState([]);
  const [selectedClasses, setSelectedClasses] = useState(new Set());
  const [classStats, setClassStats] = useState({});

  // Initialize selections when classes are detected
  useEffect(() => {
      ...
      <div style={{ flex: 1 }}>
        <div className="parameter-item">
          ...
          <div style={{ display: 'flex', gap: '5px', marginTop: '5px' }}>
            ...
            />
            <button onClick={handleFolderSelect} style={{ padding: '5px 10px' }}>
              Browse
            </button>
          </div>
        </div>
          ...
          <div style={{ marginTop: '20px' }}>
              ...
              {detectedClasses.map((className, index) => (
                <div 
                  ...
                  style={{ 
                    margin: '8px 0', 
                    padding: '8px',
                    backgroundColor: selectedClasses.has(index) ? '#f0f8ff' : '#fff',
            ...
            {detectedClasses.length === 0 && (
              <div style={{ 
                ...
                No image classes detected. Please select a folder with organized image subdirectories.
              </div>
            )}
          </div>
        )}
